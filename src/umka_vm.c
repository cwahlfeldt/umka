...
static const char *opcodeSpelling[] = {"NOP",
                                       "PUSH",
                                       "PUSH_ZERO",
                                       "PUSH_LOCAL_PTR",
                                       "PUSH_LOCAL_PTR_ZERO",
...
static const char *builtinSpelling[] = {
    ...
    "sscanf",    "real",      "real_lhs",   "round",       "trunc",
    "ceil",      "floor",     "abs",        "fabs",        "sqrt",
    "sin",       "cos",       "atan",       "atan2",       "exp",
    "log",       "new",       "make",       "makefromarr", "makefromstr",
    "maketoarr", "maketostr", "copy",       "append",      "insert",
    "delete",    "slice",     "sort",       "sortfast",    "len",
    "cap",       "sizeof",    "sizeofself", "selfhasptr",  "selftypeeq",
    "typeptr",   "valid",     "validkey",   "keys",        "resume",
    "memusage",  "exit"};

static const char *regSpelling[] = {"RESULT", "SELF", "HEAP_COPY",
                                    "SWITCH_EXPR", "EXPR_LIST"};
    ...
    if (page->ptr) {
      ...
      if (warnLeak) {
        ...
        for (int i = 0; i < page->numOccupiedChunks; i++) {
          HeapChunkHeader *chunk =
              (HeapChunkHeader *)((char *)page->ptr + i * page->chunkSize);
          if (chunk->refCnt == 0)
            continue;

          DebugInfo *debug = &pages->fiber->debugPerInstr[chunk->ip];
          fprintf(stderr, "    Chunk allocated in %s: %s (%d)\n", debug->fnName,
                  debug->fileName, debug->line);
        }
#endif
      }
      ...
      for (int i = 0;
           i < page->numOccupiedChunks && page->numChunksWithOnFree > 0; i++) {
        HeapChunkHeader *chunk =
            (HeapChunkHeader *)((char *)page->ptr + i * page->chunkSize);
...
static FORCE_INLINE HeapPage *pageAdd(HeapPages *pages, int numChunks,
                                      int chunkSize) {
...
static FORCE_INLINE HeapChunkHeader *pageGetChunkHeader(HeapPage *page,
                                                        void *ptr) {
...
static FORCE_INLINE HeapPage *pageFind(HeapPages *pages, void *ptr,
  ...
  for (HeapPage *page = pages->first; page; page = page->next)
    if (ptr >= page->ptr &&
        ptr < (void *)((char *)page->ptr + page->numChunks * page->chunkSize)) {
      HeapChunkHeader *chunk = pageGetChunkHeader(page, ptr);

      if (warnDangling && chunk->refCnt == 0)
        pages->error->runtimeHandler(pages->error->context, ERR_RUNTIME,
                                     "Dangling pointer at %p", ptr);
...
static FORCE_INLINE HeapPage *pageFindForAlloc(HeapPages *pages, int size) {
  ...
  for (HeapPage *page = pages->first; page; page = page->next)
    if (page->numOccupiedChunks < page->numChunks && page->chunkSize >= size &&
        page->chunkSize < bestSize) {
      bestPage = page;
...
static FORCE_INLINE void stackChangeFrameRefCnt(Fiber *fiber, HeapPages *pages,
                                                void *ptr, int delta) {
  if (ptr >= (void *)fiber->top &&
      ptr < (void *)(fiber->stack + fiber->stackSize)) {
    Slot *base = fiber->base;
    const ParamLayout *paramLayout = base[-2].ptrVal;

    while (ptr > (void *)(base + 1 +
                          paramLayout->numParamSlots)) // + 1 for return address
    {
      if (!stackUnwind(fiber, &base, NULL))
        pages->error->runtimeHandler(pages->error->context, ERR_RUNTIME,
                                     "Illegal stack pointer");
...
static FORCE_INLINE void *chunkAlloc(HeapPages *pages, int64_t size, Type *type,
                                     ExternFunc onFree, bool isStack,
                                     Error *error) {
  // Page layout: header, data, footer (char), padding, header, data, footer
  // (char), padding...
  int64_t chunkSize =
      align(sizeof(HeapChunkHeader) + align(size + 1, sizeof(int64_t)),
            MEM_MIN_HEAP_CHUNK);
  ...
  HeapChunkHeader *chunk =
      (HeapChunkHeader *)((char *)page->ptr +
                          page->numOccupiedChunks * page->chunkSize);
...
static FORCE_INLINE int chunkChangeRefCnt(HeapPages *pages, HeapPage *page,
                                          void *ptr, int delta) {
  HeapChunkHeader *chunk = pageGetChunkHeader(page, ptr);

  if (chunk->refCnt <= 0 || page->refCnt < chunk->refCnt)
    pages->error->runtimeHandler(pages->error->context, ERR_RUNTIME,
                                 "Wrong reference count for pointer at %p",
                                 ptr);
  ...
  // Additional ref counts for a user-defined address interval (used for stack
  // frames to detect escaping refs)
  stackChangeFrameRefCnt(pages->fiber, pages, ptr, delta);

#ifdef UMKA_REF_CNT_DEBUG
  printf("%p: delta: %d  chunk: %d  page: %d\n", ptr, delta, chunk->refCnt,
         page->refCnt);
#endif
...
static FORCE_INLINE void candidateInit(RefCntChangeCandidates *candidates) {
  ...
  candidates->stack =
      malloc(candidates->capacity * sizeof(RefCntChangeCandidate));
  candidates->top = -1;
}
...
static FORCE_INLINE void candidatePush(RefCntChangeCandidates *candidates,
                                       void *ptr, Type *type) {
  if (candidates->top >= candidates->capacity - 1) {
    candidates->capacity *= 2;
    candidates->stack =
        realloc(candidates->stack,
                candidates->capacity * sizeof(RefCntChangeCandidate));
  }
...
candidatePushDeferred(RefCntChangeCandidates *candidates, void *ptr, Type *type,
                      HeapPage *page) {
  candidatePush(candidates, ptr, type);
  candidates->stack[candidates->top].pageForDeferred = page;
}

static FORCE_INLINE void candidatePop(RefCntChangeCandidates *candidates,
                                      void **ptr, Type **type,
                                      HeapPage **page) {
  RefCntChangeCandidate *candidate = &candidates->stack[candidates->top--];
  *ptr = candidate->ptr;
...
static int fsnprintf(bool string, void *stream, int size, const char *format,
  ...
  int res = string ? vsnprintf((char *)stream, size, format, args)
...
char FORCE_INLINE *qsortPartition(char *first, char *last, int itemSize,
                                  QSortCompareFn compare, void *context,
                                  void *temp) {
  char *i = first;
  char *j = last;
...
void qsortEx(char *first, char *last, int itemSize, QSortCompareFn compare,
             void *context, void *temp) {
  if (first >= last)
    return;

  char *partition =
      qsortPartition(first, last, itemSize, compare, context, temp);
...
void vmInit(VM *vm, int stackSize, bool fileSystemEnabled, Error *error) {
  ...
  vm->fiber->stack =
      chunkAlloc(&vm->pages, stackSize * sizeof(Slot), NULL, NULL, true, error);
...
void vmFree(VM *vm) {
  ...
  if (!page)
    vm->error->runtimeHandler(vm->error->context, ERR_RUNTIME,
                              "No fiber stack");
...
void vmReset(VM *vm, Instruction *code, DebugInfo *debugPerInstr) {
  ...
  vm->fiber->ip = 0;
  vm->fiber->top = vm->fiber->base =
      vm->fiber->stack + vm->fiber->stackSize - 1;
}
...
static FORCE_INLINE void doCheckStr(const char *str, Error *error) {
  ...
  if (dims->len != strlen(str) || dims->capacity < dims->len + 1)
    error->runtimeHandler(error->context, ERR_RUNTIME, "Invalid string: %s",
                          str);
#endif
}

static FORCE_INLINE void doHook(Fiber *fiber, HookFunc *hooks,
                                HookEvent event) {
...
static FORCE_INLINE void doDerefImpl(Slot *slot, TypeKind typeKind,
                                     Error *error) {
  if (!slot->ptrVal)
  ...
  switch (typeKind) {
  case TYPE_INT8:
    slot->intVal = *(int8_t *)slot->ptrVal;
    break;
  case TYPE_INT16:
    slot->intVal = *(int16_t *)slot->ptrVal;
    break;
  case TYPE_INT32:
    slot->intVal = *(int32_t *)slot->ptrVal;
    break;
  case TYPE_INT:
    slot->intVal = *(int64_t *)slot->ptrVal;
    break;
  case TYPE_UINT8:
    slot->intVal = *(uint8_t *)slot->ptrVal;
    break;
  case TYPE_UINT16:
    slot->intVal = *(uint16_t *)slot->ptrVal;
    break;
  case TYPE_UINT32:
    slot->intVal = *(uint32_t *)slot->ptrVal;
    break;
  case TYPE_UINT:
    slot->uintVal = *(uint64_t *)slot->ptrVal;
    break;
  case TYPE_BOOL:
    slot->intVal = *(bool *)slot->ptrVal;
    break;
  case TYPE_CHAR:
    slot->intVal = *(unsigned char *)slot->ptrVal;
    break;
  case TYPE_REAL32:
    slot->realVal = *(float *)slot->ptrVal;
    break;
  case TYPE_REAL:
    slot->realVal = *(double *)slot->ptrVal;
    break;
  case TYPE_PTR:
    slot->ptrVal = *(void **)slot->ptrVal;
    break;
  case TYPE_WEAKPTR:
    slot->weakPtrVal = *(uint64_t *)slot->ptrVal;
    break;
  case TYPE_STR: {
    slot->ptrVal = *(void **)slot->ptrVal;
  ...
  case TYPE_INTERFACE:
  case TYPE_CLOSURE:
    break; // Always represented by pointer, not dereferenced
  case TYPE_FIBER:
    slot->ptrVal = *(void **)slot->ptrVal;
    break;
  case TYPE_FN:
...
static FORCE_INLINE void doAssignImpl(void *lhs, Slot rhs, TypeKind typeKind,
                                      int structSize, Error *error) {
  if (!lhs)
    error->runtimeHandler(error->context, ERR_RUNTIME, "Pointer is null");

  Const rhsConstant = {.intVal = rhs.intVal};
  if (typeOverflow(typeKind, rhsConstant))
    error->runtimeHandler(error->context, ERR_RUNTIME, "Overflow of %s",
                          typeKindSpelling(typeKind));

  switch (typeKind) {
  case TYPE_INT8:
    *(int8_t *)lhs = rhs.intVal;
    break;
  case TYPE_INT16:
    *(int16_t *)lhs = rhs.intVal;
    break;
  case TYPE_INT32:
    *(int32_t *)lhs = rhs.intVal;
    break;
  case TYPE_INT:
    *(int64_t *)lhs = rhs.intVal;
    break;
  case TYPE_UINT8:
    *(uint8_t *)lhs = rhs.intVal;
    break;
  case TYPE_UINT16:
    *(uint16_t *)lhs = rhs.intVal;
    break;
  case TYPE_UINT32:
    *(uint32_t *)lhs = rhs.intVal;
    break;
  case TYPE_UINT:
    *(uint64_t *)lhs = rhs.uintVal;
    break;
  case TYPE_BOOL:
    *(bool *)lhs = rhs.intVal;
    break;
  case TYPE_CHAR:
    *(unsigned char *)lhs = rhs.intVal;
    break;
  case TYPE_REAL32:
    *(float *)lhs = rhs.realVal;
    break;
  case TYPE_REAL:
    *(double *)lhs = rhs.realVal;
    break;
  case TYPE_PTR:
    *(void **)lhs = rhs.ptrVal;
    break;
  case TYPE_WEAKPTR:
    *(uint64_t *)lhs = rhs.weakPtrVal;
    break;
  case TYPE_STR: {
...
static FORCE_INLINE void
doAddPtrBaseRefCntCandidate(RefCntChangeCandidates *candidates, void *ptr,
                            Type *type) {
  if (typeKindGarbageCollected(type->base->kind)) {
    void *data = ptr;
    if (type->base->kind == TYPE_PTR || type->base->kind == TYPE_STR ||
        type->base->kind == TYPE_FIBER)
      data = *(void **)data;
...
static FORCE_INLINE void
doAddArrayItemsRefCntCandidates(RefCntChangeCandidates *candidates, void *ptr,
                                Type *type, int len) {
  if (typeKindGarbageCollected(type->base->kind)) {
    char *itemPtr = ptr;
...
static FORCE_INLINE void
doAddStructFieldsRefCntCandidates(RefCntChangeCandidates *candidates, void *ptr,
                                  Type *type) {
  for (int i = 0; i < type->numItems; i++) {
    if (typeKindGarbageCollected(type->field[i]->type->kind)) {
      ...
      if (type->field[i]->type->kind == TYPE_PTR ||
          type->field[i]->type->kind == TYPE_STR ||
          type->field[i]->type->kind == TYPE_FIBER)
        field = *(void **)field;
...
static FORCE_INLINE void doChangeRefCntImpl(Fiber *fiber, HeapPages *pages,
                                            void *ptr, Type *type,
                                            TokenKind tokKind) {
  // Update ref counts for pointers (including static/dynamic array items and
  // structure/interface fields) if allocated dynamically All garbage collected
  // composite types are represented by pointers by default RTTI is required for
  // lists, trees, etc., since the propagation depth for the root ref count is
  ...
  while (candidates->top >= 0) {
    ...
    // Process deferred ref count updates first (the heap page should have been
    // memoized for them)
    if (pageForDeferred) {
      chunkChangeRefCnt(pages, pageForDeferred, ptr,
                        (tokKind == TOK_PLUSPLUS) ? 1 : -1);
      continue;
    }
    ...
    case TYPE_PTR: {
      ...
      if (tokKind == TOK_PLUSPLUS)
        ...
        // Only one ref is left. Defer processing the parent and traverse the
        // children before removing the ref
        candidatePushDeferred(candidates, ptr, type, page);

        // Sometimes the last remaining ref to chunk data is a pointer to a
        // single item of a composite type (interior pointer) In this case, we
        // should traverse children as for the actual composite type, rather
        // than for the pointer
        if (chunk->type) {
          void *chunkDataPtr = (char *)chunk + sizeof(HeapChunkHeader);

          switch (chunk->type->kind) {
          case TYPE_ARRAY: {
            doAddArrayItemsRefCntCandidates(candidates, chunkDataPtr,
                                            chunk->type, chunk->type->numItems);
            break;
          }
          case TYPE_DYNARRAY: {
            DynArrayDimensions *dims = (DynArrayDimensions *)chunkDataPtr;
            void *data = (char *)chunkDataPtr + sizeof(DynArrayDimensions);
            doAddArrayItemsRefCntCandidates(candidates, data, chunk->type,
                                            dims->len);
            break;
          }
          case TYPE_STRUCT: {
            doAddStructFieldsRefCntCandidates(candidates, chunkDataPtr,
                                              chunk->type);
            break;
          }
          default: {
    ...
    case TYPE_DYNARRAY: {
      ...
      if (tokKind == TOK_PLUSPLUS)
        ...
        doAddArrayItemsRefCntCandidates(candidates, array->data, type,
                                        getDims(array)->len);
      }
      break;
    }
...
static FORCE_INLINE char *doAllocStr(HeapPages *pages, int64_t len,
                                     Error *error) {
  StrDimensions dims = {.len = len, .capacity = 2 * (len + 1)};

  char *dimsAndData = chunkAlloc(pages, sizeof(StrDimensions) + dims.capacity,
...
static FORCE_INLINE void doAllocDynArray(HeapPages *pages, DynArray *array,
                                         Type *type, int64_t len,
                                         Error *error) {
  array->type = type;
  ...
  char *dimsAndData = chunkAlloc(
      pages, sizeof(DynArrayDimensions) + dims.capacity * array->itemSize,
      array->type, NULL, false, error);
  *(DynArrayDimensions *)dimsAndData = dims;
...
static FORCE_INLINE void doAllocMap(HeapPages *pages, Map *map, Type *type,
                                    Error *error) {
  map->type = type;
  map->root = chunkAlloc(pages, typeSizeNoCheck(type->base), type->base, NULL,
                         false, error);
  map->root->len = 0;
}

static void doGetMapKeyBytes(Slot key, Type *keyType, Error *error,
                             char **keyBytes, int *keySize) {
  switch (keyType->kind) {
  case TYPE_INT8:
  case TYPE_INT16:
  case TYPE_INT32:
...
static FORCE_INLINE void doRebalanceMapNodes(MapNode **nodeInParent) {
  ...
  if (node && !node->left && node->right && !node->right->left &&
      node->right->right) {
  ...
  if (node && node->left && !node->right && node->left->left &&
      !node->left->right) {
...
static FORCE_INLINE MapNode **doGetMapNode(Map *map, Slot key,
                                           bool createMissingNodes,
                                           HeapPages *pages, Error *error) {
  if (!map || !map->root)
  ...
  if (keySize == 0)
    error->runtimeHandler(error->context, ERR_RUNTIME,
                          "Map key has zero length");
  ...
  while (*node) {
    ...
    if ((*node)->key) {
      ...
      doGetMapKeyBytes(nodeKeySlot, typeMapKey(map->type), error, &nodeKeyBytes,
                       &nodeKeySize);
    }
  ...
  if (createMissingNodes) {
    ...
    *node = (MapNode *)chunkAlloc(pages, typeSizeNoCheck(nodeType), nodeType,
                                  NULL, false, error);
  }
...
static MapNode *doCopyMapNode(Map *map, MapNode *node, Fiber *fiber,
                              HeapPages *pages, Error *error) {
  if (!node)
    return NULL;

  Type *nodeType = map->type->base;
  MapNode *result = (MapNode *)chunkAlloc(pages, typeSizeNoCheck(nodeType),
                                          nodeType, NULL, false, error);
  ...
  if (node->key) {
    ...
    // When allocating dynamic arrays, we mark with type the data chunk, not the
    // header chunk
    result->key = chunkAlloc(pages, typeSizeNoCheck(keyType),
                             keyType->kind == TYPE_DYNARRAY ? NULL : keyType,
                             NULL, false, error);
  ...
  if (node->data) {
    ...
    // When allocating dynamic arrays, we mark with type the data chunk, not the
    // header chunk
    result->data = chunkAlloc(pages, typeSizeNoCheck(itemType),
                              itemType->kind == TYPE_DYNARRAY ? NULL : itemType,
                              NULL, false, error);
...
static void doGetMapKeysRecursively(Map *map, MapNode *node, void *keys,
                                    int *numKeys, Error *error) {
  if (node->left)
...
static FORCE_INLINE void doGetMapKeys(Map *map, void *keys, Error *error) {
  ...
  if (numKeys != map->root->len)
    error->runtimeHandler(error->context, ERR_RUNTIME,
                          "Wrong number of map keys");
}

static FORCE_INLINE Fiber *doAllocFiber(Fiber *parent, Closure *childClosure,
                                        Type *childClosureType,
                                        HeapPages *pages, Error *error) {
  ...
  *child = *parent;
  child->stack = chunkAlloc(pages, child->stackSize * sizeof(Slot), NULL, NULL,
                            true, error);
  ...
  *(Interface *)child->top = childClosure->upvalue;
  doChangeRefCntImpl(child, pages, child->top, childClosureSig->param[0]->type,
                     TOK_PLUSPLUS);
...
static FORCE_INLINE int doPrintIndented(char *buf, int maxLen, int depth,
                                        bool pretty, char ch) {
  enum { INDENT_WIDTH = 4 };
...
static int doFillReprBuf(Slot *slot, Type *type, char *buf, int maxLen,
                         int depth, bool pretty, bool dereferenced,
                         Error *error) {
  enum { MAX_DEPTH = 20 };
  ...
  switch (type->kind) {
  case TYPE_VOID:
    len += snprintf(buf + len, maxLen, "void");
    break;
  case TYPE_INT8:
  case TYPE_INT16:
  case TYPE_INT32:
  ...
  case TYPE_UINT16:
  case TYPE_UINT32:
    len += snprintf(buf + len, maxLen, "%lld", (long long int)slot->intVal);
    break;
  case TYPE_UINT:
    len += snprintf(buf + len, maxLen, "%llu",
                    (unsigned long long int)slot->uintVal);
    break;
  case TYPE_BOOL:
    len += snprintf(buf + len, maxLen, slot->intVal ? "true" : "false");
    break;
  case TYPE_CHAR: {
  ...
  case TYPE_REAL32:
  case TYPE_REAL:
    len += snprintf(buf + len, maxLen, "%lg", slot->realVal);
    break;
  case TYPE_PTR: {
    ...
    if (dereferenced && slot->ptrVal && type->base->kind != TYPE_VOID) {
      ...
      len += doFillReprBuf(&dataSlot, type->base, buf + len, maxLen, depth + 1,
                           pretty, dereferenced, error);
      len += doPrintIndented(buf + len, maxLen, depth, pretty, ')');
    }
    break;
  }
  case TYPE_WEAKPTR:
    len += snprintf(buf + len, maxLen, "%llx",
                    (unsigned long long int)slot->weakPtrVal);
    break;
  case TYPE_STR: {
    doCheckStr((char *)slot->ptrVal, error);
    len += snprintf(buf + len, maxLen, "\"%s\"",
                    slot->ptrVal ? (char *)slot->ptrVal : "");
    break;
  }
  case TYPE_ARRAY: {
    len += doPrintIndented(buf + len, maxLen, depth, pretty, '[');
    ...
    for (int i = 0; i < type->numItems; i++) {
      ...
      len += doFillReprBuf(&itemSlot, type->base, buf + len, maxLen, depth + 1,
                           pretty, dereferenced, error);
  ...
  case TYPE_DYNARRAY: {
    ...
    if (array && array->data) {
      ...
      for (int i = 0; i < getDims(array)->len; i++) {
        Slot itemSlot = {.ptrVal = itemPtr};
        doDerefImpl(&itemSlot, type->base->kind, error);
        len += doFillReprBuf(&itemSlot, type->base, buf + len, maxLen,
                             depth + 1, pretty, dereferenced, error);
  ...
  case TYPE_MAP: {
    ...
    if (map && map->root) {
      ...
      for (int i = 0; i < map->root->len; i++) {
        ...
        len += doFillReprBuf(&keySlot, keyType, buf + len, maxLen, depth + 1,
                             pretty, dereferenced, error);
        ...
        len += doFillReprBuf(&itemSlot, itemType, buf + len, maxLen, depth + 1,
                             pretty, dereferenced, error);
  ...
  case TYPE_CLOSURE: {
    ...
    for (int i = 0; i < type->numItems; i++) {
      Slot fieldSlot = {.ptrVal =
                            (char *)slot->ptrVal + type->field[i]->offset};
      doDerefImpl(&fieldSlot, type->field[i]->type->kind, error);
      if (!skipNames)
        len += snprintf(buf + len, maxLen, "%s: ", type->field[i]->name);
      len += doFillReprBuf(&fieldSlot, type->field[i]->type, buf + len, maxLen,
                           depth + 1, pretty, dereferenced, error);
  ...
  case TYPE_INTERFACE: {
    ...
    if (interface->self) {
      ...
      if (pretty) {
        ...
        len += snprintf(buf + len, maxLen, "%s",
                        typeSpelling(interface->selfType->base, selfTypeBuf));
        len += doPrintIndented(buf + len, maxLen, depth, pretty, '(');
      }

      len += doFillReprBuf(&selfSlot, interface->selfType->base, buf + len,
                           maxLen, depth + 1, pretty, dereferenced, error);
  ...
  case TYPE_FIBER:
    len += snprintf(buf + len, maxLen, "fiber @ %p", slot->ptrVal);
    break;
  case TYPE_FN:
    len +=
        snprintf(buf + len, maxLen, "fn @ %lld", (long long int)slot->intVal);
    break;
  default:
    break;
  }
...
static FORCE_INLINE void doCheckFormatString(const char *format, int *formatLen,
                                             int *typeLetterPos,
                                             TypeKind *typeKind,
                                             FormatStringTypeSize *size,
                                             Error *error) {
    ...
    *typeKind = TYPE_VOID;
    ...
    if (format[i] == '%') {
      ...
      *typeLetterPos = i;
      ...
      case 'v':
        *typeKind = TYPE_INTERFACE; /* Actually any type */
        break;

      default:
        error->runtimeHandler(error->context, ERR_RUNTIME,
                              "Illegal type character %c in format string",
                              format[i]);
      }
      i++;
    }
...
static FORCE_INLINE int doPrintSlot(bool string, void *stream, int maxLen,
                                    const char *format, Slot slot,
                                    TypeKind typeKind, Error *error) {
  int len = -1;

  switch (typeKind) {
  case TYPE_VOID:
    len = fsnprintf(string, stream, maxLen, format);
    break;
  case TYPE_INT8:
    len = fsnprintf(string, stream, maxLen, format, (int8_t)slot.intVal);
    break;
  case TYPE_INT16:
    len = fsnprintf(string, stream, maxLen, format, (int16_t)slot.intVal);
    break;
  case TYPE_INT32:
    len = fsnprintf(string, stream, maxLen, format, (int32_t)slot.intVal);
    break;
  case TYPE_INT:
    len = fsnprintf(string, stream, maxLen, format, slot.intVal);
    break;
  case TYPE_UINT8:
    len = fsnprintf(string, stream, maxLen, format, (uint8_t)slot.intVal);
    break;
  case TYPE_UINT16:
    len = fsnprintf(string, stream, maxLen, format, (uint16_t)slot.intVal);
    break;
  case TYPE_UINT32:
    len = fsnprintf(string, stream, maxLen, format, (uint32_t)slot.intVal);
    break;
  case TYPE_UINT:
    len = fsnprintf(string, stream, maxLen, format, slot.uintVal);
    break;
  case TYPE_BOOL:
    len = fsnprintf(string, stream, maxLen, format, (bool)slot.intVal);
    break;
  case TYPE_CHAR:
    len = fsnprintf(string, stream, maxLen, format, (unsigned char)slot.intVal);
    break;
  case TYPE_REAL32:
  case TYPE_REAL:
    len = fsnprintf(string, stream, maxLen, format, slot.realVal);
    break;
  case TYPE_STR: {
    doCheckStr((char *)slot.ptrVal, error);
    len = fsnprintf(string, stream, maxLen, format,
...
static FORCE_INLINE void doBuiltinPrintf(Fiber *fiber, HeapPages *pages,
                                         bool console, bool string,
                                         Error *error) {
  ...
  doCheckFormatString(format, &formatLen, &typeLetterPos, &expectedTypeKind,
                      &formatStringTypeSize, error);

  const bool hasAnyTypeFormatter = expectedTypeKind == TYPE_INTERFACE &&
                                   typeLetterPos >= 0 &&
                                   typeLetterPos < formatLen; // %v

  if (type->kind != expectedTypeKind &&
      !(type->kind != TYPE_VOID && expectedTypeKind == TYPE_INTERFACE) &&
      !(typeKindIntegerOrEnum(type->kind) &&
        typeKindIntegerOrEnum(expectedTypeKind)) &&
      !(typeKindReal(type->kind) && typeKindReal(expectedTypeKind))) {
    char typeBuf[DEFAULT_STR_LEN + 1];
    error->runtimeHandler(
        error->context, ERR_RUNTIME, "Incompatible types %s and %s in printf()",
        typeKindSpelling(expectedTypeKind), typeSpelling(type, typeBuf));
  }
  ...
  if (expectedTypeKind != TYPE_VOID) {
    ...
    if (typeConvOverflow(expectedTypeKind, type->kind, arg))
      error->runtimeHandler(error->context, ERR_RUNTIME, "Overflow of %s",
                            typeKindSpelling(expectedTypeKind));
  }

  char curFormatBuf[DEFAULT_STR_LEN + 1];
  bool isCurFormatBufInHeap = formatLen + 1 > sizeof(curFormatBuf);
  char *curFormat =
      isCurFormatBufInHeap ? malloc(formatLen + 1) : &curFormatBuf;

  memcpy(curFormat, format, formatLen);
  curFormat[formatLen] = 0;

  // Special case: %v formatter - convert argument of any type to its string
  // representation
  ...
  if (hasAnyTypeFormatter) {
    ...
    if ((formatStringTypeSize == FORMAT_SIZE_LONG_LONG ||
         formatStringTypeSize == FORMAT_SIZE_SHORT_SHORT) &&
        typeLetterPos - 2 >= 0)
      curFormat[typeLetterPos - 2] = ' ';

    const bool pretty = formatStringTypeSize == FORMAT_SIZE_LONG_LONG;
    const bool dereferenced = formatStringTypeSize == FORMAT_SIZE_LONG ||
                              formatStringTypeSize == FORMAT_SIZE_LONG_LONG;

    const int reprLen =
        doFillReprBuf(&value, type, NULL, 0, 0, pretty, dereferenced,
                      error); // Predict buffer length

    isReprBufInHeap = sizeof(StrDimensions) + reprLen + 1 > sizeof(reprBuf);
    dimsAndRepr = isReprBufInHeap ? malloc(sizeof(StrDimensions) + reprLen + 1)
                                  : &reprBuf;
    ...
    *(StrDimensions *)dimsAndRepr = dims;
    ...
    doFillReprBuf(&value, type, repr, reprLen + 1, 0, pretty, dereferenced,
                  error); // Fill buffer
  ...
  if (string) {
    ...
    const bool inPlace =
        stream && getStrDims(stream)->capacity >= prevLen + len + 1;
    if (inPlace) {
    ...
    len = doPrintSlot(true, (char *)stream + prevLen, len + 1, curFormat, value,
                      typeKind, error);
  } else
    len =
        doPrintSlot(false, stream, INT_MAX, curFormat, value, typeKind, error);

  fiber->top[STACK_OFFSET_FORMAT].ptrVal =
      (char *)fiber->top[STACK_OFFSET_FORMAT].ptrVal + formatLen;
...
static FORCE_INLINE void doBuiltinScanf(Fiber *fiber, HeapPages *pages,
                                        bool console, bool string,
                                        Error *error) {
  void *stream =
      console ? stdin : (void *)fiber->top[STACK_OFFSET_STREAM].ptrVal;
  const char *format = (const char *)fiber->top[STACK_OFFSET_FORMAT].ptrVal;
  Slot value = fiber->top[STACK_OFFSET_VALUE];
  ...
  if (typeKind != expectedTypeKind || expectedTypeKind == TYPE_INTERFACE) {
    error->runtimeHandler(
        error->context, ERR_RUNTIME, "Incompatible types %s and %s in scanf()",
        typeKindSpelling(expectedTypeKind), typeKindSpelling(typeKind));
  }

  char curFormatBuf[DEFAULT_STR_LEN + 1];
  bool isCurFormatBufInHeap =
      formatLen + 2 + 1 > sizeof(curFormatBuf); // + 2 for "%n"
  char *curFormat =
      isCurFormatBufInHeap ? malloc(formatLen + 2 + 1) : &curFormatBuf;
  ...
  if (string)
    fiber->top[STACK_OFFSET_STREAM].ptrVal =
        (char *)fiber->top[STACK_OFFSET_STREAM].ptrVal + len;
...
static FORCE_INLINE void doBuiltinNew(Fiber *fiber, HeapPages *pages,
                                      Error *error) {
...
static FORCE_INLINE void doBuiltinMake(Fiber *fiber, HeapPages *pages,
                                       Error *error) {
  Type *type = fiber->code[fiber->ip].type;
...
static FORCE_INLINE void doBuiltinMakefromarr(Fiber *fiber, HeapPages *pages,
                                              Error *error) {
  ...
  Type staticArrayType = {.kind = TYPE_ARRAY,
                          .base = dest->type->base,
                          .numItems = getDims(dest)->len,
                          .next = NULL};
...
static FORCE_INLINE void doBuiltinMakefromstr(Fiber *fiber, HeapPages *pages,
                                              Error *error) {
  DynArray *dest = (DynArray *)(fiber->top++)->ptrVal;
...
static FORCE_INLINE void doBuiltinMaketoarr(Fiber *fiber, HeapPages *pages,
                                            Error *error) {
  ...
  if (src->data) {
    if (getDims(src)->len > destType->numItems)
      error->runtimeHandler(error->context, ERR_RUNTIME,
                            "Dynamic array is too long");

    memcpy(dest, src->data, getDims(src)->len * src->itemSize);

    // Increase result items' ref counts, as if they have been assigned one by
    // one
...
static FORCE_INLINE void doBuiltinMaketostr(Fiber *fiber, HeapPages *pages,
                                            Error *error) {
...
static FORCE_INLINE void doBuiltinCopyDynArray(Fiber *fiber, HeapPages *pages,
                                               Error *error) {
  DynArray *result = (DynArray *)(fiber->top++)->ptrVal;
  ...
  *result = *array;
  ...
  if (array->data) {
    ...
    memmove((char *)result->data, (char *)array->data,
            getDims(array)->len * array->itemSize);

    // Increase result items' ref counts, as if they have been assigned one by
    // one
    Type staticArrayType = {.kind = TYPE_ARRAY,
                            .base = result->type->base,
                            .numItems = getDims(result)->len,
                            .next = NULL};
    doChangeRefCntImpl(fiber, pages, result->data, &staticArrayType,
                       TOK_PLUSPLUS);
  }
...
static FORCE_INLINE void doBuiltinCopyMap(Fiber *fiber, HeapPages *pages,
                                          Error *error) {
  Map *result = (Map *)(fiber->top++)->ptrVal;
...
static FORCE_INLINE void doBuiltinCopy(Fiber *fiber, HeapPages *pages,
                                       Error *error) {
  Type *type = fiber->code[fiber->ip].type;
  if (type->kind == TYPE_DYNARRAY)
...
static FORCE_INLINE void doBuiltinAppend(Fiber *fiber, HeapPages *pages,
                                         Error *error) {
  DynArray *result = (DynArray *)(fiber->top++)->ptrVal;
  ...
  if (newLen <= getDims(array)->capacity) {
    ...
    *result = *array;
    ...
    memmove((char *)result->data + getDims(array)->len * array->itemSize,
            (char *)rhs, rhsLen * array->itemSize);
    ...
    Type staticArrayType = {.kind = TYPE_ARRAY,
                            ...
                            .numItems = rhsLen,
                            .next = NULL};
    doChangeRefCntImpl(fiber, pages,
                       (char *)result->data +
                           getDims(array)->len * array->itemSize,
                       &staticArrayType, TOK_PLUSPLUS);
...
static FORCE_INLINE void doBuiltinInsert(Fiber *fiber, HeapPages *pages,
                                         Error *error) {
  DynArray *result = (DynArray *)(fiber->top++)->ptrVal;
  void *item = (fiber->top++)->ptrVal;
  ...
  if (index < 0 || index > getDims(array)->len)
    error->runtimeHandler(error->context, ERR_RUNTIME,
                          "Index %lld is out of range 0...%lld", index,
                          getDims(array)->len);
  ...
  if (getDims(array)->len + 1 <= getDims(array)->capacity) {
    ...
    *result = *array;
    ...
    memmove((char *)result->data + (index + 1) * result->itemSize,
            (char *)result->data + index * result->itemSize,
            (getDims(array)->len - index) * result->itemSize);
    memmove((char *)result->data + index * result->itemSize, (char *)item,
            result->itemSize);
    ...
    memmove((char *)result->data, (char *)array->data,
            index * result->itemSize);
    memmove((char *)result->data + (index + 1) * result->itemSize,
            (char *)array->data + index * result->itemSize,
            (getDims(array)->len - index) * result->itemSize);
    memmove((char *)result->data + index * result->itemSize, (char *)item,
            result->itemSize);
...
static FORCE_INLINE void doBuiltinDeleteDynArray(Fiber *fiber, HeapPages *pages,
                                                 Error *error) {
  DynArray *result = (DynArray *)(fiber->top++)->ptrVal;
  int64_t index = (fiber->top++)->intVal;
  DynArray *array = (DynArray *)(fiber->top++)->ptrVal;
  ...
  *result = *array;
  ...
  doChangeRefCntImpl(fiber, pages,
                     (char *)result->data + index * result->itemSize,
                     &staticArrayType, TOK_MINUSMINUS);

  memmove((char *)result->data + index * result->itemSize,
          (char *)result->data + (index + 1) * result->itemSize,
          (getDims(array)->len - index - 1) * result->itemSize);
...
static FORCE_INLINE void doBuiltinDeleteMap(Fiber *fiber, HeapPages *pages,
                                            Error *error) {
  Map *result = (Map *)(fiber->top++)->ptrVal;
  Slot key = *fiber->top++;
  Map *map = (Map *)(fiber->top++)->ptrVal;
  ...
  if (node) {
    ...
    doChangeRefCntImpl(fiber, pages, node, typeMapNodePtr(map->type),
                       TOK_MINUSMINUS);

    if (--map->root->len < 0)
      error->runtimeHandler(error->context, ERR_RUNTIME,
                            "Map length is negative");
  }

  doChangeRefCntImpl(fiber, pages, map->root, typeMapNodePtr(map->type),
                     TOK_PLUSPLUS);
  result->type = map->type;
...
static FORCE_INLINE void doBuiltinDelete(Fiber *fiber, HeapPages *pages,
                                         Error *error) {
  Type *type = fiber->code[fiber->ip].type;
  if (type->kind == TYPE_DYNARRAY)
...
static FORCE_INLINE void doBuiltinSlice(Fiber *fiber, HeapPages *pages,
                                        Error *error) {
  DynArray *result = (DynArray *)(fiber->top++)->ptrVal;
  ...
  if (startIndex < 0)
    error->runtimeHandler(error->context, ERR_RUNTIME,
                          "Index %lld is out of range 0...%lld", startIndex,
                          len);

  if (endIndex < startIndex || endIndex > len)
    error->runtimeHandler(error->context, ERR_RUNTIME,
                          "Index %lld is out of range %lld...%lld", endIndex,
                          startIndex, len);
  ...
  if (result) {
    ...
    memcpy((char *)result->data,
           (char *)array->data + startIndex * result->itemSize,
           getDims(result)->len * result->itemSize);
...
static int qsortCompare(const void *a, const void *b, void *context) {
  ...
  *(Interface *)fiber->top = compare->upvalue;
  doChangeRefCntImpl(fiber, &fiber->vm->pages, fiber->top,
                     compareSig->param[0]->type, TOK_PLUSPLUS);

  // Push pointers to values to be compared
  (--fiber->top)->ptrVal = (void *)a;
  doChangeRefCntImpl(fiber, &fiber->vm->pages, fiber->top->ptrVal,
                     compareSig->param[1]->type, TOK_PLUSPLUS);

  (--fiber->top)->ptrVal = (void *)b;
  doChangeRefCntImpl(fiber, &fiber->vm->pages, fiber->top->ptrVal,
                     compareSig->param[2]->type, TOK_PLUSPLUS);
...
static FORCE_INLINE void doBuiltinSort(Fiber *fiber, HeapPages *pages,
                                       Error *error) {
  ...
  if (array->data && getDims(array)->len > 0) {
    ...
    const int numTempSlots =
        align(array->itemSize, sizeof(Slot)) / sizeof(Slot);
    fiber->top -= numTempSlots;

    qsortEx((char *)array->data,
            (char *)array->data + array->itemSize * (getDims(array)->len - 1),
            array->itemSize, qsortCompare, &context, fiber->top);
...
static int qsortFastCompare(const void *a, const void *b, void *context) {
  ...
  case TYPE_INT8: {
    int64_t diff = (int64_t)(*(int8_t *)lhs) - (int64_t)(*(int8_t *)rhs);
    return diff == 0 ? 0 : diff > 0 ? sign : -sign;
  }
  case TYPE_INT16: {
    int64_t diff = (int64_t)(*(int16_t *)lhs) - (int64_t)(*(int16_t *)rhs);
    return diff == 0 ? 0 : diff > 0 ? sign : -sign;
  }
  case TYPE_INT32: {
  ...
  case TYPE_UINT8: {
    int64_t diff = (int64_t)(*(uint8_t *)lhs) - (int64_t)(*(uint8_t *)rhs);
    return diff == 0 ? 0 : diff > 0 ? sign : -sign;
  }
  case TYPE_UINT16: {
    int64_t diff = (int64_t)(*(uint16_t *)lhs) - (int64_t)(*(uint16_t *)rhs);
    return diff == 0 ? 0 : diff > 0 ? sign : -sign;
  }
  case TYPE_UINT32: {
  ...
  case TYPE_BOOL: {
    int64_t diff = (int64_t)(*(bool *)lhs) - (int64_t)(*(bool *)rhs);
    return diff == 0 ? 0 : diff > 0 ? sign : -sign;
  }
  case TYPE_CHAR: {
    int64_t diff =
        (int64_t)(*(unsigned char *)lhs) - (int64_t)(*(unsigned char *)rhs);
    return diff == 0 ? 0 : diff > 0 ? sign : -sign;
  }
  case TYPE_REAL32: {
    double diff = (double)(*(float *)lhs) - (double)(*(float *)rhs);
    return diff == 0 ? 0 : diff > 0 ? sign : -sign;
  }
  case TYPE_REAL: {
...
static FORCE_INLINE void doBuiltinSortfast(Fiber *fiber, HeapPages *pages,
                                           Error *error) {
  ...
  if (array->data && getDims(array)->len > 0) {
    ...
    qsortEx((char *)array->data,
            (char *)array->data + array->itemSize * (getDims(array)->len - 1),
            array->itemSize, qsortFastCompare, &context, fiber->top);
...
static FORCE_INLINE void doBuiltinValidkey(Fiber *fiber, HeapPages *pages,
                                           Error *error) {
  Slot key = *fiber->top++;
  Map *map = (Map *)(fiber->top++)->ptrVal;
...
static FORCE_INLINE void doBuiltinKeys(Fiber *fiber, HeapPages *pages,
                                       Error *error) {
  DynArray *result = (DynArray *)(fiber->top++)->ptrVal;
  Map *map = (Map *)(fiber->top++)->ptrVal;
  ...
  doAllocDynArray(pages, result, resultType, map->root ? map->root->len : 0,
                  error);
...
static FORCE_INLINE void doBuiltinResume(Fiber *fiber, Fiber **newFiber,
                                         HeapPages *pages, Error *error) {
  Fiber *child = (Fiber *)(fiber->top++)->ptrVal;
...
// fn memusage(): int
static FORCE_INLINE void doBuiltinMemusage(Fiber *fiber, HeapPages *pages,
                                           Error *error) {
...
static FORCE_INLINE void doPushLocalPtr(Fiber *fiber) {
  (--fiber->top)->ptrVal =
      (int8_t *)fiber->base + fiber->code[fiber->ip].operand.intVal;
  fiber->ip++;
}

static FORCE_INLINE void doPushLocalPtrZero(Fiber *fiber) {
  (--fiber->top)->ptrVal =
      (int8_t *)fiber->base + fiber->code[fiber->ip].operand.int32Val[0];
...
static FORCE_INLINE void doPushReg(Fiber *fiber) {
  (--fiber->top)->intVal =
      fiber->reg[fiber->code[fiber->ip].operand.intVal].intVal;
  fiber->ip++;
}

static FORCE_INLINE void doPushUpvalue(Fiber *fiber, HeapPages *pages,
                                       Error *error) {
  Closure *closure = (Closure *)(fiber->top++)->ptrVal;
...
static FORCE_INLINE void doAssign(Fiber *fiber, Error *error) {
  ...
  doAssignImpl(lhs, rhs, fiber->code[fiber->ip].typeKind,
               fiber->code[fiber->ip].operand.intVal, error);
  fiber->ip++;
}
...
static FORCE_INLINE void doAssignParam(Fiber *fiber, Error *error) {
  ...
  doAssignImpl(fiber->top, rhs, fiber->code[fiber->ip].typeKind, paramSize,
               error);
  fiber->ip++;
}
...
static FORCE_INLINE void doChangeRefCntGlobal(Fiber *fiber, HeapPages *pages,
                                              Error *error) {
  TokenKind tokKind = fiber->code[fiber->ip].tokKind;
  Type *type = fiber->code[fiber->ip].type;
...
static FORCE_INLINE void doChangeRefCntLocal(Fiber *fiber, HeapPages *pages,
                                             Error *error) {
  TokenKind tokKind = fiber->code[fiber->ip].tokKind;
  Type *type = fiber->code[fiber->ip].type;
...
static FORCE_INLINE void doChangeRefCntAssign(Fiber *fiber, HeapPages *pages,
                                              Error *error) {
  if (fiber->code[fiber->ip].inlineOpcode == OP_SWAP)
    doSwapImpl(fiber->top);
  ...
  if (fiber->code[fiber->ip].tokKind !=
      TOK_MINUSMINUS) // "--" means that the right-hand side ref count should
                      // not be increased
...
static FORCE_INLINE void doUnary(Fiber *fiber, Error *error) {
  if (fiber->code[fiber->ip].typeKind == TYPE_REAL ||
      fiber->code[fiber->ip].typeKind == TYPE_REAL32)
    switch (fiber->code[fiber->ip].tokKind) {
    case TOK_PLUS:
      break;
    case TOK_MINUS:
      fiber->top->realVal = -fiber->top->realVal;
      break;
    default:
      error->runtimeHandler(error->context, ERR_RUNTIME, "Illegal instruction");
      return;
    }
  else
    switch (fiber->code[fiber->ip].tokKind) {
    case TOK_PLUS:
      break;
    case TOK_MINUS:
      fiber->top->intVal = -fiber->top->intVal;
      break;
    case TOK_NOT:
      fiber->top->intVal = !fiber->top->intVal;
      break;
    ...
    case TOK_PLUSPLUS: {
      ...
      case TYPE_UINT:
        (*(uint64_t *)ptr)++;
        break;
      // Structured, boolean, char and real types are not
...
static FORCE_INLINE void doBinary(Fiber *fiber, HeapPages *pages,
                                  Error *error) {
  Slot rhs = *fiber->top++;
  Slot *lhs = fiber->top;
  ...
  if (fiber->code[fiber->ip].typeKind == TYPE_PTR) {
    switch (fiber->code[fiber->ip].tokKind) {
    case TOK_EQEQ:
      lhs->intVal = lhs->ptrVal == rhs.ptrVal;
      break;
    case TOK_NOTEQ:
      lhs->intVal = lhs->ptrVal != rhs.ptrVal;
      break;
    ...
    case TOK_PLUSEQ: {
      ...
      const bool inPlace = fiber->code[fiber->ip].tokKind == TOK_PLUSEQ &&
                           getStrDims(lhsStr)->capacity >= lhsLen + rhsLen + 1;
    ...
    case TOK_EQEQ:
      lhs->intVal = strcmp(lhsStr, rhsStr) == 0;
      break;
    case TOK_NOTEQ:
      lhs->intVal = strcmp(lhsStr, rhsStr) != 0;
      break;
  ...
  } else if (fiber->code[fiber->ip].typeKind == TYPE_ARRAY ||
             fiber->code[fiber->ip].typeKind == TYPE_STRUCT) {
    const int structSize = fiber->code[fiber->ip].operand.intVal;

    switch (fiber->code[fiber->ip].tokKind) {
    case TOK_EQEQ:
      lhs->intVal = memcmp(lhs->ptrVal, rhs.ptrVal, structSize) == 0;
      break;
    case TOK_NOTEQ:
      lhs->intVal = memcmp(lhs->ptrVal, rhs.ptrVal, structSize) != 0;
      break;
    ...
    case TOK_EQEQ:
      lhs->intVal = lhs->realVal == rhs.realVal;
      break;
    case TOK_NOTEQ:
      lhs->intVal = lhs->realVal != rhs.realVal;
      break;
    ...
    case TOK_EQEQ:
      lhs->uintVal = lhs->uintVal == rhs.uintVal;
      break;
    case TOK_NOTEQ:
      lhs->uintVal = lhs->uintVal != rhs.uintVal;
      break;
  ...
  } else // All ordinal types except TYPE_UINT
  {
    ...
    case TOK_EQEQ:
      lhs->intVal = lhs->intVal == rhs.intVal;
      break;
    case TOK_NOTEQ:
      lhs->intVal = lhs->intVal != rhs.intVal;
      break;
...
static FORCE_INLINE void doGetArrayPtr(Fiber *fiber, Error *error) {
  ...
  } else // For strings, negative length means that the actual string length is
         // to be used
  {
    if (!data)
      data = doGetEmptyStr();
  ...
  if (index < 0 || index > len - 1)
    error->runtimeHandler(error->context, ERR_RUNTIME,
                          "Index %d is out of range 0...%d", index, len - 1);
...
static FORCE_INLINE void doGetMapPtr(Fiber *fiber, HeapPages *pages,
                                     Error *error) {
  Slot key = *fiber->top++;
  Map *map = (Map *)(fiber->top++)->ptrVal;
  ...
  if (!node->data) {
    // When allocating dynamic arrays, we mark with type the data chunk, not the
    // header chunk
    node->key = chunkAlloc(pages, typeSizeNoCheck(keyType),
                           keyType->kind == TYPE_DYNARRAY ? NULL : keyType,
                           NULL, false, error);
    node->data = chunkAlloc(pages, typeSizeNoCheck(itemType),
                            itemType->kind == TYPE_DYNARRAY ? NULL : itemType,
                            NULL, false, error);
    ...
    doAssignImpl(node->key, key, keyType->kind, typeSizeNoCheck(keyType),
                 error);
    map->root->len++;
  }
...
static FORCE_INLINE void doAssertType(Fiber *fiber) {
  ...
  (--fiber->top)->ptrVal =
      (interface->selfType && typeEquivalent(type, interface->selfType))
          ? interface->self
          : NULL;
  fiber->ip++;
}
...
static FORCE_INLINE void doCallIndirect(Fiber *fiber, Error *error) {
  // For indirect calls, entry point address is below the parameters on the
  // stack
...
static FORCE_INLINE void doCallExtern(Fiber *fiber, Error *error) {
  ...
  fiber->reg[REG_RESULT].ptrVal =
      error->context; // Upon entry, the result slot stores the Umka instance

  int ip = fiber->ip;
  fn(fiber->base + 2,
     &fiber->reg[REG_RESULT]); // + 2 for old base pointer and return address
  fiber->ip = ip;

  fiber->ip++;
}

static FORCE_INLINE void doCallBuiltin(Fiber *fiber, Fiber **newFiber,
                                       HeapPages *pages, Error *error) {
  // Preserve instruction pointer, in case any of the standard calls end up
  // calling Umka again
  int ip = fiber->ip;
  ...
  // I/O
  case BUILTIN_PRINTF:
    doBuiltinPrintf(fiber, pages, true, false, error);
    break;
  case BUILTIN_FPRINTF:
    doBuiltinPrintf(fiber, pages, false, false, error);
    break;
  case BUILTIN_SPRINTF:
    doBuiltinPrintf(fiber, pages, false, true, error);
    break;
  case BUILTIN_SCANF:
    doBuiltinScanf(fiber, pages, true, false, error);
    break;
  case BUILTIN_FSCANF:
    doBuiltinScanf(fiber, pages, false, false, error);
    break;
  case BUILTIN_SSCANF:
    doBuiltinScanf(fiber, pages, false, true, error);
    break;
  ...
  case BUILTIN_ROUND:
    fiber->top->intVal = (int64_t)round(fiber->top->realVal);
    break;
  case BUILTIN_TRUNC:
    fiber->top->intVal = (int64_t)trunc(fiber->top->realVal);
    break;
  case BUILTIN_CEIL:
    fiber->top->intVal = (int64_t)ceil(fiber->top->realVal);
    break;
  case BUILTIN_FLOOR:
    fiber->top->intVal = (int64_t)floor(fiber->top->realVal);
    break;
  case BUILTIN_ABS: {
  ...
  case BUILTIN_FABS:
    fiber->top->realVal = fabs(fiber->top->realVal);
    break;
  case BUILTIN_SQRT: {
  ...
  case BUILTIN_SIN:
    fiber->top->realVal = sin(fiber->top->realVal);
    break;
  case BUILTIN_COS:
    fiber->top->realVal = cos(fiber->top->realVal);
    break;
  case BUILTIN_ATAN:
    fiber->top->realVal = atan(fiber->top->realVal);
    break;
  case BUILTIN_ATAN2: {
  ...
  case BUILTIN_EXP:
    fiber->top->realVal = exp(fiber->top->realVal);
    break;
  case BUILTIN_LOG: {
    if (fiber->top->realVal <= 0)
  ...
  // Memory
  case BUILTIN_NEW:
    doBuiltinNew(fiber, pages, error);
    break;
  case BUILTIN_MAKE:
    doBuiltinMake(fiber, pages, error);
    break;
  case BUILTIN_MAKEFROMARR:
    doBuiltinMakefromarr(fiber, pages, error);
    break;
  case BUILTIN_MAKEFROMSTR:
    doBuiltinMakefromstr(fiber, pages, error);
    break;
  case BUILTIN_MAKETOARR:
    doBuiltinMaketoarr(fiber, pages, error);
    break;
  case BUILTIN_MAKETOSTR:
    doBuiltinMaketostr(fiber, pages, error);
    break;
  case BUILTIN_COPY:
    doBuiltinCopy(fiber, pages, error);
    break;
  case BUILTIN_APPEND:
    doBuiltinAppend(fiber, pages, error);
    break;
  case BUILTIN_INSERT:
    doBuiltinInsert(fiber, pages, error);
    break;
  case BUILTIN_DELETE:
    doBuiltinDelete(fiber, pages, error);
    break;
  case BUILTIN_SLICE:
    doBuiltinSlice(fiber, pages, error);
    break;
  case BUILTIN_SORT:
    doBuiltinSort(fiber, pages, error);
    break;
  case BUILTIN_SORTFAST:
    doBuiltinSortfast(fiber, pages, error);
    break;
  case BUILTIN_LEN:
    doBuiltinLen(fiber, error);
    break;
  case BUILTIN_CAP:
    doBuiltinCap(fiber, error);
    break;
  case BUILTIN_SIZEOF:
    error->runtimeHandler(error->context, ERR_RUNTIME, "Illegal instruction");
    return; // Done at compile time
  case BUILTIN_SIZEOFSELF:
    doBuiltinSizeofself(fiber, error);
    break;
  case BUILTIN_SELFHASPTR:
    doBuiltinSelfhasptr(fiber, error);
    break;
  case BUILTIN_SELFTYPEEQ:
    doBuiltinSelftypeeq(fiber, error);
    break;
  case BUILTIN_TYPEPTR:
    error->runtimeHandler(error->context, ERR_RUNTIME, "Illegal instruction");
    return; // Done at compile time
  case BUILTIN_VALID:
    doBuiltinValid(fiber, error);
    break;

  // Maps
  case BUILTIN_VALIDKEY:
    doBuiltinValidkey(fiber, pages, error);
    break;
  case BUILTIN_KEYS:
    doBuiltinKeys(fiber, pages, error);
    break;

  // Fibers
  case BUILTIN_RESUME:
    doBuiltinResume(fiber, newFiber, pages, error);
    break;

  // Misc
  case BUILTIN_MEMUSAGE:
    doBuiltinMemusage(fiber, pages, error);
    break;
  case BUILTIN_EXIT:
    doBuiltinExit(fiber, error);
    return;
  }
...
static FORCE_INLINE void doReturn(Fiber *fiber, Fiber **newFiber) {
  ...
  if (returnOffset == RETURN_FROM_FIBER) {
    // For fiber function, kill the fiber, extract the parent fiber pointer and
    // switch to it
    fiber->alive = false;
    *newFiber = fiber->parent;
  } else {
...
static FORCE_INLINE void doEnterFrame(Fiber *fiber, HeapPages *pages,
                                      HookFunc *hooks, Error *error) {
...
static FORCE_INLINE void vmLoop(VM *vm) {
  ...
  while (1) {
    ...
    case OP_PUSH:
      ...
      break;
    case OP_PUSH_ZERO:
      doPushZero(fiber, error);
      break;
    case OP_PUSH_LOCAL_PTR:
      doPushLocalPtr(fiber);
      break;
    case OP_PUSH_LOCAL_PTR_ZERO:
      doPushLocalPtrZero(fiber);
      break;
    case OP_PUSH_LOCAL:
      doPushLocal(fiber, error);
      break;
    case OP_PUSH_REG:
      doPushReg(fiber);
      break;
    case OP_PUSH_UPVALUE:
      doPushUpvalue(fiber, pages, error);
      break;
    case OP_POP:
      doPop(fiber);
      break;
    ...
    case OP_DEREF:
      ...
      break;
    case OP_ASSIGN:
      doAssign(fiber, error);
      break;
    case OP_ASSIGN_PARAM:
      doAssignParam(fiber, error);
      break;
    case OP_CHANGE_REF_CNT:
      doChangeRefCnt(fiber, pages);
      break;
    case OP_CHANGE_REF_CNT_GLOBAL:
      doChangeRefCntGlobal(fiber, pages, error);
      break;
    case OP_CHANGE_REF_CNT_LOCAL:
      doChangeRefCntLocal(fiber, pages, error);
      break;
    case OP_CHANGE_REF_CNT_ASSIGN:
      doChangeRefCntAssign(fiber, pages, error);
      break;
    case OP_UNARY:
      doUnary(fiber, error);
      break;
    case OP_BINARY:
      doBinary(fiber, pages, error);
      break;
    case OP_GET_ARRAY_PTR:
      doGetArrayPtr(fiber, error);
      break;
    case OP_GET_DYNARRAY_PTR:
      doGetDynArrayPtr(fiber, error);
      break;
    case OP_GET_MAP_PTR:
      doGetMapPtr(fiber, pages, error);
      break;
    case OP_GET_FIELD_PTR:
      doGetFieldPtr(fiber, error);
      break;
    case OP_ASSERT_TYPE:
      doAssertType(fiber);
      break;
    case OP_ASSERT_RANGE:
      doAssertRange(fiber, error);
      break;
    case OP_WEAKEN_PTR:
      doWeakenPtr(fiber, pages);
      break;
    case OP_STRENGTHEN_PTR:
      doStrengthenPtr(fiber, pages);
      break;
    case OP_GOTO:
      doGoto(fiber);
      break;
    ...
    case OP_CALL_INDIRECT:
      doCallIndirect(fiber, error);
      break;
    case OP_CALL_EXTERN:
      doCallExtern(fiber, error);
      break;
    ...
    case OP_ENTER_FRAME:
      doEnterFrame(fiber, pages, hooks, error);
      break;
    case OP_LEAVE_FRAME:
      doLeaveFrame(fiber, pages, hooks, error);
      break;
    case OP_HALT:
      doHalt(vm);
      return;
  ...
  if (fn) {
    ...
    if (fn->params) {
      const ParamLayout *paramLayout =
          (ParamLayout *)fn->params[-4]
              .ptrVal; // For -4, see the stack layout diagram in umka_vm.c
...
bool vmAlive(VM *vm) { return vm->mainFiber->alive; }

void vmKill(VM *vm) { vm->mainFiber->alive = false; }

int vmAsm(int ip, Instruction *code, DebugInfo *debugPerInstr, char *buf,
          int size) {
  Instruction *instr = &code[ip];
  DebugInfo *debug = &debugPerInstr[ip];

  char opcodeBuf[DEFAULT_STR_LEN + 1];
  snprintf(opcodeBuf, DEFAULT_STR_LEN + 1, "%s%s",
           instr->inlineOpcode == OP_SWAP ? "SWAP; " : "",
           opcodeSpelling[instr->opcode]);
  int chars = snprintf(buf, size, "%09d %6d %28s", ip, debug->line, opcodeBuf);

  if (instr->tokKind != TOK_NONE)
    chars += snprintf(buf + chars, nonneg(size - chars), " %s",
                      lexSpelling(instr->tokKind));

  if (instr->type) {
    char typeBuf[DEFAULT_STR_LEN + 1];
    chars += snprintf(buf + chars, nonneg(size - chars), " %s",
                      typeSpelling(instr->type, typeBuf));
  }

  if (instr->typeKind != TYPE_NONE &&
      (!instr->type || instr->opcode == OP_ASSERT_RANGE))
    chars += snprintf(buf + chars, nonneg(size - chars), " %s",
                      typeKindSpelling(instr->typeKind));

  switch (instr->opcode) {
  case OP_PUSH: {
    if (instr->typeKind == TYPE_PTR || instr->inlineOpcode == OP_DEREF)
      chars += snprintf(buf + chars, nonneg(size - chars), " %p",
                        instr->operand.ptrVal);
    else if (instr->typeKind == TYPE_REAL)
      chars += snprintf(buf + chars, nonneg(size - chars), " %lg",
                        instr->operand.realVal);
    else
      chars += snprintf(buf + chars, nonneg(size - chars), " %lld",
                        (long long int)instr->operand.intVal);
    break;
  }
  case OP_PUSH_REG:
  case OP_POP_REG:
    chars += snprintf(buf + chars, nonneg(size - chars), " %s",
                      regSpelling[instr->operand.intVal]);
    break;
  case OP_PUSH_ZERO:
  case OP_PUSH_LOCAL_PTR:
  case OP_PUSH_LOCAL:
  ...
  case OP_CALL: {
    ...
    chars += snprintf(buf + chars, nonneg(size - chars), " %s (%lld)", fnName,
                      (long long int)instr->operand.intVal);
    break;
  }
  case OP_PUSH_LOCAL_PTR_ZERO:
  case OP_GET_ARRAY_PTR:
    chars += snprintf(buf + chars, nonneg(size - chars), " %d %d",
                      (int)instr->operand.int32Val[0],
                      (int)instr->operand.int32Val[1]);
    break;
  case OP_CHANGE_REF_CNT_GLOBAL:
  case OP_ENTER_FRAME:
  case OP_CALL_EXTERN:
  ...
  case OP_CALL_BUILTIN:
    chars += snprintf(buf + chars, nonneg(size - chars), " %s",
                      builtinSpelling[instr->operand.builtinVal]);
    break;
...
int64_t vmGetMemUsage(VM *vm) { return vm->pages.totalSize; }
